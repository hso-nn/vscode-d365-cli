...
export class Lint {
    public static currentPanel: Lint | undefined;
    ...
    public static render(extensionUri: Uri) {
      if (Lint.currentPanel) {
        Lint.currentPanel._panel.reveal(ViewColumn.One);
      } else {
        ...
        Lint.currentPanel = new Lint(panel, extensionUri);
      }
    }

    public dispose() {
      Lint.currentPanel = undefined;
        this._panel.dispose();
    ...
    private _getWebviewContent(webview: Webview, extensionUri: Uri) {
      const webviewUri = getUri(webview, extensionUri, ["out", "lint.js"]);
      ...
      return /*html*/ `
        ...
        <html lang="en">
          <head>
            ...
            <title>HSO D365 CLI Extension - Lint</title>
          </head>
          <body>
          <img nonce="${nonce}" src="${image}" alt="HSO Logo" height="50">
          <h1>HSO D365 Extension</h1>
          <h2>Lint</h2>
          <p>The project contains a .eslintrc.json file for linting configurations. We recommend to use the recommendations as much as possible.</p>
          <p>Command 'lint' does linting on the project.</p>
