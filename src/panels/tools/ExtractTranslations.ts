...
export class ExtractTranslations {
    public static currentPanel: ExtractTranslations | undefined;
    ...
    public static render(extensionUri: Uri) {
      if (ExtractTranslations.currentPanel) {
        ExtractTranslations.currentPanel._panel.reveal(ViewColumn.One);
      } else {
        ...
        ExtractTranslations.currentPanel = new ExtractTranslations(panel, extensionUri);
      }
    }

    public dispose() {
      ExtractTranslations.currentPanel = undefined;
        this._panel.dispose();
    ...
    private _getWebviewContent(webview: Webview, extensionUri: Uri) {
      const webviewUri = getUri(webview, extensionUri, ["out", "extracttranslations.js"]);
      ...
      return /*html*/ `
        ...
        <html lang="en">
          <head>
            ...
            <title>HSO D365 CLI Extension - Extract Translations</title>
          </head>
          <body>
          <img nonce="${nonce}" src="${image}" alt="HSO Logo" height="50">
          <h1>HSO D365 Extension</h1>
          <h2>Extract Translations</h2>
          <p>Command 'extractTranslations' extracts all translation from project TypeScript files.</p>
