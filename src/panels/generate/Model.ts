...
export class Model {
    public static currentPanel: Model | undefined;
    ...
    public static render(extensionUri: Uri) {
      if (Model.currentPanel) {
        Model.currentPanel._panel.reveal(ViewColumn.One);
      } else {
        ...
        Model.currentPanel = new Model(panel, extensionUri);
      }
    }

    public dispose() {
      Model.currentPanel = undefined;
        this._panel.dispose();
    ...
    private _getWebviewContent(webview: Webview, extensionUri: Uri) {
      const webviewUri = getUri(webview, extensionUri, ["out", "model.js"]);
      ...
      return /*html*/ `
        ...
        <html lang="en">
          <head>
            ...
            <title>HSO D365 CLI Extension - Generate Model</title>
          </head>
          <body>
          <img nonce="${nonce}" src="${image}" alt="HSO Logo" height="50">
          <h1>HSO D365 Extension</h1>
          <h2>Generate Model</h2>
          <p>Command 'generate Model [name]' generates a Model for an entity. The model file will contain all attributes for the $select and $expand part of odata.</p>
          <vscode-text-field id="modelName">Model Name</vscode-text-field>&nbsp;<i>The name of the model</i><br><br>
          <vscode-text-field id="modelLogicalName">Logical Name</vscode-text-field>&nbsp;<i>The logical name of the entity</i><br><br>
