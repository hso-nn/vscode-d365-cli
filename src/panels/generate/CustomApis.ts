...
export class CustomApis {
    public static currentPanel: CustomApis | undefined;
    ...
    public static render(extensionUri: Uri) {
      if (CustomApis.currentPanel) {
        CustomApis.currentPanel._panel.reveal(ViewColumn.One);
      } else {
        ...
        CustomApis.currentPanel = new CustomApis(panel, extensionUri);
      }
    }

    public dispose() {
      CustomApis.currentPanel = undefined;
        this._panel.dispose();
    ...
    private _getWebviewContent(webview: Webview, extensionUri: Uri) {
      const webviewUri = getUri(webview, extensionUri, ["out", "customapis.js"]);
      ...
      return /*html*/ `
        ...
        <html lang="en">
          <head>
            ...
            <title>HSO D365 CLI Extension - Generate CustomApis</title>
          </head>
          <body>
          <img nonce="${nonce}" src="${image}" alt="HSO Logo" height="50">
          <h1>HSO D365 Extension</h1>
          <h2>Generate Custom Apis</h2>
          <p>Command 'generate CustomApis generates an CustomApis file. The CustomApis file will contain all Custom API's.</p>
